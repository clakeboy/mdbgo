...
func (mdb *MDB) QueryContext(ctx context.Context, sqlText string, params map[string]any) (*Rows, error) {
	if ctx == nil {
	...
	return mdb.queryStatement(ctx, stmt, params)
}

func (mdb *MDB) queryStatement(ctx context.Context, stmt *sqlSelect, params map[string]any) (*Rows, error) {
	if ctx == nil {
		ctx = context.Background()
	}
...
func (mdb *MDB) QueryViewContext(ctx context.Context, viewName string, params map[string]any) (*Rows, error) {
	...
	sqlText, err := mdb.ViewSQL(viewName)
	if err != nil {
		return nil, err
	}
	stmt, err := parseAccessSQLCached(sqlText)
...
func (e *queryExecutor) catalog() error {
	...
	e.tableNames = make(map[string]string)
	e.viewNames = make(map[string]string)
	// Views() and Tables() both call ReadCatalog which destroys and repopulates
	// the handle's catalog. Call Views() first (Query type), then Tables() last
	// (Table type) so that the catalog ends up with Table entries for subsequent
	// GetTableSchema/GetCatalogEntryByName lookups.
	views, err := e.mdb.Views()
	if err != nil {
		return err
	}
	for _, v := range views {
		e.viewNames[strings.ToLower(v)] = v
	}
	tables, err := e.mdb.Tables()
	if err != nil {
		return err
	}
	for _, t := range tables {
		e.tableNames[strings.ToLower(t)] = t
	}
	return nil
}
...
func (e *queryExecutor) loadSource(ref sqlTableRef, maxRows int, requestedColumns []string) (*queryRelation, error) {
	...
	if name, ok := e.viewNames[key]; ok {
		...
		sqlText, err := e.mdb.ViewSQL(name)
		if err != nil {
			return nil, err
		}
		stmt, err := parseAccessSQLCached(sqlText)
...
func (e *queryExecutor) loadPhysicalTable(name, alias string, maxRows int, requestedColumns []string) (*queryRelation, error) {
	schema, err := e.mdb.GetTableSchema(name)
	if err != nil {
		return nil, fmt.Errorf("loadPhysicalTable GetTableSchema(%q): %w", name, err)
	}
	...
	data, err := e.mdb.readProjectedData(name, requestedColumns, maxRows)
	if err != nil {
		return nil, err
	}
	source := alias
	...
	for i, col := range data.Columns {
		...
		for _, schemaCol := range schema.Columns {
			if strings.EqualFold(schemaCol.Name, col) {
				kind = valueKindForAccessType(schemaCol.Type)
				typeName = schemaCol.TypeName
				break
			}
		}
		rel.columns[i] = queryColumn{name: col, source: source, tableName: name, kind: kind, typeName: typeName}
	}
	...
	for _, item := range items {
		if star, ok := item.expr.(sqlStar); ok {
			for _, col := range source.columns {
				if star.qualifier == "" || strings.EqualFold(star.qualifier, col.source) || strings.EqualFold(star.qualifier, col.tableName) {
...
func (e *queryExecutor) resolveIdent(ident sqlIdent, row []Value, rel *queryRelation) (Value, error) {
	...
	found := -1
	for i, col := range rel.columns {
		if strings.EqualFold(col.name, name) && (source == "" || strings.EqualFold(col.source, source) || strings.EqualFold(col.tableName, source)) {
			if found >= 0 {
